...
#  OPT  : Steam / Gaming / linux-surface kernel
# =============================================================================
# HOW TO USE:
...
# ── linux-surface kernel & iptsd touchscreen ──────────────────────────────────
# The Microsoft Surface Pro 8's touchscreen, pen, cameras, and power buttons
# require the linux-surface kernel patches.

# Add linux-surface repository for pre-built Surface kernel
curl -s https://pkg.surfacelinux.com/arch/sign.gpg | pacman-key --add -
pacman-key --finger 56C464BAAC421952
...
# Generate initramfs for the surface kernel
mkinitcpio -p linux-surface

# Surface boot entry
SURFACE_UUID=\$(blkid -s UUID -o value ${PART_ROOT})
cat > /boot/efi/loader/entries/arch-surface.conf <<EOF
title   Arch Linux — linux-surface (Surface Pro 8) [RECOMMENDED]
linux   /vmlinuz-linux-surface
initrd  /intel-ucode.img
initrd  /initramfs-linux-surface.img
options root=UUID=\${SURFACE_UUID} rw quiet splash
        mem_sleep_default=s2idle
        i915.enable_psr=1
        i915.enable_fbc=1
        mitigations=off
...
cp /boot/vmlinuz-linux-surface            /boot/efi/
cp /boot/initramfs-linux-surface.img      /boot/efi/
cp /boot/initramfs-linux-surface-fallback.img /boot/efi/ 2>/dev/null || true

# Set Surface kernel as default boot entry
sed -i 's/^default.*/default  arch-surface.conf/' /boot/efi/loader/loader.conf
...
echo -e "${BOLD}${GREEN}║  2. Reboot — select 'linux-surface' entry at boot        ║${NC}"
