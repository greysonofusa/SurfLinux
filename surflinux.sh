...
# Show the user every block device on the system with full details FIRST.
# This is the only safe way to make an informed disk selection — the user
# needs to see ALL disks before choosing, not just be shown a pre-selected one.
echo -e "${BOLD}All detected storage devices:${NC}"
echo
lsblk -dpno NAME,SIZE,MODEL,TYPE,TRAN | awk '$4=="disk"' | column -t
echo

# Build a list of all physical disks (no partitions, loops, or rom drives)
mapfile -t DISK_LIST < <(lsblk -dpno NAME,TYPE,TRAN | awk '$2=="disk" {print $1}')

[[ ${#DISK_LIST[@]} -eq 0 ]] && die "No physical disks detected. Cannot continue."

# Interactive selection — present a numbered menu, require explicit user choice.
# This completely replaces the auto-guess logic. The user MUST consciously pick.
echo -e "${BOLD}Available disks:${NC}"
for i in "${!DISK_LIST[@]}"; do
    DEV="${DISK_LIST[$i]}"
    SIZE=$(lsblk -dpno SIZE "$DEV" 2>/dev/null || echo "?")
    MODEL=$(lsblk -dpno MODEL "$DEV" 2>/dev/null | xargs || echo "Unknown")
    TRAN=$(lsblk -dpno TRAN "$DEV" 2>/dev/null | xargs || echo "")
    printf "  ${CYAN}[%d]${NC}  %-14s  %8s  %s  %s\n" \
        "$((i+1))" "$DEV" "$SIZE" "$MODEL" "$TRAN"
done
echo

# If DISK was pre-set at the top of the script, validate it exists in the list
# and still ask for explicit confirmation — never skip the interactive prompt.
if [[ -n "$DISK" ]]; then
    if lsblk "$DISK" &>/dev/null; then
        warn "DISK is pre-set to '${DISK}' in the script config."
    else
        warn "Pre-set DISK='${DISK}' not found. Ignoring — please select below."
        DISK=""
    fi
fi

while true; do
    read -rp "$(echo -e "${BOLD}Enter disk number (1-${#DISK_LIST[@]}) or full path (e.g. /dev/nvme0n1): ${NC}")" DISK_INPUT

    # Accept a number from the menu
    if [[ "$DISK_INPUT" =~ ^[0-9]+$ ]]; then
        IDX=$(( DISK_INPUT - 1 ))
        if (( IDX >= 0 && IDX < ${#DISK_LIST[@]} )); then
            DISK="${DISK_LIST[$IDX]}"
            break
        else
            warn "Invalid selection. Choose 1–${#DISK_LIST[@]}."
        fi
    # Accept a full /dev/ path — but validate it's actually in our detected list
    elif [[ "$DISK_INPUT" == /dev/* ]]; then
        if lsblk "$DISK_INPUT" &>/dev/null 2>&1; then
            DISK="$DISK_INPUT"
            break
        else
            warn "'${DISK_INPUT}' is not a valid block device."
        fi
    else
        warn "Enter a number or a full device path like /dev/nvme0n1."
    fi
done

# Show the chosen disk in detail so the user can see exactly what they selected
echo
echo -e "${BOLD}Selected disk:${NC}"
lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT,MODEL "$DISK"
echo

# Double-confirmation — type the device name itself, not just "yes".
# Forces the user to read and type the actual device path they're about to wipe.
echo -e "${RED}${BOLD}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${RED}${BOLD}║  !!  ALL DATA ON ${DISK} WILL BE PERMANENTLY DESTROYED  !!  ║${NC}"
echo -e "${RED}${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
echo
read -rp "$(echo -e "${RED}${BOLD}To confirm, type the full device name '${DISK}': ${NC}")" CONFIRM
